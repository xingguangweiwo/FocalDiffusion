...
class DefocusConsistencyModule(nn.Module):
    """
    ...
    def __init__(
        ...
        if use_differentiable_rendering:
            self.defocus_renderer = ThinLensDefocusRenderer(
    ...
    def forward(
        ...
        Args:
            ...
            focal_plane_distances: Focus distances [B, N]
        ...
        if self.use_defocus_consistency:
            rendered_stack = self.render_defocused_focal_stack(
                all_in_focus, depth, focal_plane_distances, camera_params
            )
        ...
        coc_maps = self.compute_coc_maps(depth, focal_plane_distances, camera_params)

        # Ensure CoC is physically plausible
        coc_loss = self.compute_coc_response_consistency_loss(coc_maps, focal_stack)
        ...
        ordering_loss = self.compute_depth_ordering_loss(depth, focal_stack, focal_plane_distances)
    ...
    def render_defocused_focal_stack(
            self,
            all_in_focus: torch.Tensor,
            depth: torch.Tensor,
            focal_plane_distances: torch.Tensor,
        ...
        if not self.use_differentiable_rendering:
            return torch.zeros_like(all_in_focus).unsqueeze(1).expand(-1, len(focal_plane_distances), -1, -1, -1)

        B = all_in_focus.shape[0]
        N = focal_plane_distances.shape[1]

        rendered_images = []

        for n in range(N):
            focus_d = focal_plane_distances[:, n]

            # Compute CoC for this focus distance
            coc = self.compute_thin_lens_coc(depth, focus_d, camera_params)
    ...
    def compute_coc_maps(
        ...
        for n in range(N):
            focus_d = focal_plane_distances[:, n:n + 1].unsqueeze(-1).unsqueeze(-1)
    ...
    def compute_coc_response_consistency_loss(
            self,
    ...
    def compute_depth_ordering_loss(
        ...
        focus_at_sharpest = torch.gather(
            focal_plane_distances.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, depth.shape[-2], depth.shape[-1]),
            1,
            sharpest_idx
        )
...
class ThinLensDefocusRenderer(nn.Module):
    """
...
PhysicsConsistencyModule = DefocusConsistencyModule
DifferentiableDefocusRenderer = ThinLensDefocusRenderer
DefocusConsistencyModule.render_focal_stack = DefocusConsistencyModule.render_defocused_focal_stack
DefocusConsistencyModule.compute_single_coc = DefocusConsistencyModule.compute_thin_lens_coc
DefocusConsistencyModule.compute_coc_consistency_loss = DefocusConsistencyModule.compute_coc_response_consistency_loss
